# ============================================================
# LOCAL INTERACTIVE TRAINING SCRIPT
# Works on your local machine (Fedora/Linux)
# ============================================================
...
print("="*60)
print("🎯 LOCAL INTERACTIVE TRAINING - Myanmar Agriculture ML")
print("="*60)
...
DATASET_PATH = "combined_dataset.csv"  # Path to the combined dataset CSV
...
# Fallback resolution for dataset path
if not os.path.exists(DATASET_PATH):
    if os.path.exists("data/combined/combined_dataset.csv"):
        DATASET_PATH = "data/combined/combined_dataset.csv"
    elif os.path.exists("utils/combined_dataset.csv"):
        DATASET_PATH = "utils/combined_dataset.csv"

# Check if dataset exists
if not os.path.exists(DATASET_PATH):
    print(f"❌ Dataset not found at: {DATASET_PATH}")
    print("\nPlease update DATASET_PATH to the correct location.")
    sys.exit(1)
...
# Convert ALL_TARGETS to a string representation that can be embedded
targets_str = "{\n"
for key, value in ALL_TARGETS.items():
    targets_str += f'    "{key}": {value},\n'
targets_str += "}"
...
train_script_path = os.path.join(SCRIPT_DIR, "train_interactive.py")
...
print(f"   • Script: {train_script_path}")
